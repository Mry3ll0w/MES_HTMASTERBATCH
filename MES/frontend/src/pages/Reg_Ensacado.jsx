...
export default function RegEnsacado() {
    ...
    function InsertaEnsacado(){
        ...
        //Si todo esta correcto enviamos el post para que el backend trate la query
        if(ok){
            axios.post('http://192.168.0.123:4001/RegistraEnsacado',
            {
                Fecha :M_Fecha, Turno : M_Turno, Producto : M_Producto, Palet : M_Palet, Peso_Saco : M_Peso_Saco,
                Cantidad : M_Cantidad, Resto : M_Resto, Ant : M_Ant 
            }
    ...
    function DeleteEnsacados(){
        var err;
        Selected.map( (i) => {
            axios.post('http://192.168.0.123:4001/DelEns',{Fecha: i.Fecha, Turno: i.Turno, Palet: i.Palet})
            .catch(e => {err=e;});
        })
        if(err){
  ...
  return (
    <Fragment>
        <h1>
            Bienvenido al panel de Inserción/Eliminación de los ensacados.
        </h1>
        <p>
        ...
        {<Box sx={{p:2,m:'3px', border:'1px dotted blue'}} >
            ...
            <Button sx={{m : '10px'}} onClick={DeleteEnsacados} variant='contained'>Elimina el/los ensacado seleccionados</Button>
            </Paper>
            ...
            <p>*Nota : En caso de equivocación en la inserción del ensacado seleccione el erroneo en la lista anterior y corrijalo</p>
        </Box>}
